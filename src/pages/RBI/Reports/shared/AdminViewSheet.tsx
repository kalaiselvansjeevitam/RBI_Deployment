...
const RBIViewSheet = ({ open, workshopId, openClose }: StudentSheetProps) => {
...
const InfoRow = ({
  ...
  <div
    className={`text-sm text-gray-700 ${
      stack ? "flex flex-col gap-1" : "flex justify-between items-start gap-4"
    }`}
  >
    {/* Label */}
    ...
    <span
      className={`font-semibold break-words ${
        stack
          ? "text-left"
          : align === "left"
            ? "text-left max-w-[220px]"
            : "text-right max-w-[220px]"
      }`}
    >
      {value}
    </span>
